...
## Main Menu - Phase 2 with animated background and detailed cucumber
...
func _ready() -> void:
	...
	cucumber_preview.position = Vector2(viewport_size.x * 0.5, viewport_size.y * 0.78)
	add_child(cucumber_preview)

	# Version label bottom-right
	var version := Label.new()
	version.text = "v0.2 - Phase 2"
...
# --- Inner class: Detailed Cucumber Preview ---
...
class CucumberPreview extends Node2D:
	...
	func _draw() -> void:
		...
		if is_blinking:
			...
			var look_x: float = sin(timer * 0.7) * 2.5
			var look_y: float = cos(timer * 0.5) * 1.5
			var look := Vector2(look_x, look_y)

			draw_circle(Vector2(-7, -5) + look, 3.5, Color(0.08, 0.08, 0.08))
			draw_circle(Vector2(7, -5) + look, 3.0, Color(0.08, 0.08, 0.08))
		...
		if is_waving:
			# Waving right arm
			var wave_angle: float = sin(wave_timer * 8.0) * 0.5 + 0.3
...
class FloatingVeggie extends Node2D:
	...
	func _ready() -> void:
		veggie_type = randi() % 6  # 0=tomato, 1=carrot, 2=chili, 3=salt, 4=onion, 5=broccoli
